...
function insertRow(payload, token) {
  return fetch(SUPABASE_URL + "/rest/v1/objects", {
    ...
    // 409 = the row's primary key is already there, i.e. this entry's
    // insert already succeeded on an earlier pass and only the queue
    // cleanup after it failed. Retrying can never turn that into a 201,
    // so treating it as an error left the entry queued to fail forever.
    // It's done -- report success and let the caller delete it.
    if (r.status === 409) return;
    if (!r.ok) throw new Error("record insert failed (" + r.status + ")");
  });
}
